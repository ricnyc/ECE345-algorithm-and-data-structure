...
void insert_command(stringstream &linestream, string &line, NodeList *&Nodehead)
{
    ...
    endpoint_[1] = node2; 
    int print_once = 0;
    //insert the node and resistor in each list
...
void setV_command(stringstream &linestream, NodeList *&Nodehead)
{
    ...
    if(!Nodehead->find_node(nodeID))//not found for the NodeID
    {    
        Nodehead->find_or_insert_node(nodeID)->set_voltage(voltage);
    }
    else
    {
...
{        
    ...
    if(!Nodehead->find_node(nodeID))//not found for the NodeID
    {     
        Nodehead->find_or_insert_node(nodeID)->unset_voltage();
    }
    else
    {
...
void solve_command(stringstream &linestream, NodeList *&Nodehead)
{
     ...
     for(ptr = Nodehead->find_head_of_NodeList(); ptr != NULL; ptr = ptr->get_next_node())
     {
         ...
         if(ptr->get_set_bool() == true && flag == true)//check for nodeList which has one set, then go inside
         {
              Nodehead->solve();
